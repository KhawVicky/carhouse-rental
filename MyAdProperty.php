...
if ($_SERVER['REQUEST_METHOD'] == 'POST' &&isset($_POST['recordID']) && isset($_POST['status'])){
    ...
    if ($status === 'Available') {
      ...
      $stmt = $pdo->prepare('UPDATE house SET available = ? WHERE ID = ?');
...
$baseQuery = 'SELECT * FROM house WHERE email = :email';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST["search"])) {
    ...
    $_SESSION['userSortOptionP'] = $sortOption;
...
} elseif (isset($_SESSION['userSortOptionP'])) {
    // If sorting option is set in session, apply it
    switch ($_SESSION['userSortOptionP']) {
        case "lowest":
            $orderCondition = ' ORDER BY price ASC, ID';
            break;
        case "highest":
...
$stmt_total = $pdo->prepare('SELECT COUNT(*) AS total_records FROM house WHERE email = :email');
...
<link href="https://fonts.googleapis.com/css2?family=Roboto+Condensed:ital,wght@0,100..900;1,100..900&family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,500;1,700;1,900&display=swap" rel="stylesheet"> 
  ...
  <section id="propertySection" class="propertySection "<?php if($error===true):?>style="height:75vh"<?php endif; ?>>
      <div class="container py-5">
        <div class="row">
          <div class="col-12">
            <div class="text-center">
            <h1 class="text-dark fw-bold">My AD Properties</h1>
            <hr class="hr-heading" />
            </div>
          </div>
        </div>
...
<li class="nav-item">
  <a class="nav-link text-dark" aria-current="page" href="MyAdCar.php" >Car</a>
</li>
<li class="nav-item">
  <a class="nav-link " href="MyAdProperty.php" style="color:#074eac;">Property</a>
</li>
              ...
              <button
               ...
               <p class="text-dark fw-bold py-2 m-0 text-start">Sort By</p>
                ...
                foreach ($sorts as $sort) {
                    $checked = (!empty($_SESSION['userSortOptionP']) && $_SESSION['userSortOptionP'] === $sort) ? 'checked' : '';
                    echo "
         ...
         <?php endif; ?>
           ...
           <a href="detailsHouse.php?ID=<?= $record['ID'] ?>">
            ...
            <?php endif; ?>
                     ...
                     <a href="editProperty.php?ID=<?= $record['ID'] ?>" class="btn btn-primary btn-md mt-2">Edit Ad</a>
                    </li>
...
function updateAvailability(recordID, status) {
    ...
    xhr.open("POST", "MyAdProperty.php"); // Point to the correct PHP file
