...
if (isset($_POST['clear'])) {
  ...
  exit;*/
    ...
    $stmt = $pdo->prepare('SELECT * FROM bookmark WHERE email = :email AND category = "property"');
...
} elseif (isset($_SESSION['userSortOptionMyAdFP'])) {
    // If sorting option is set in session, apply it
    switch ($_SESSION['userSortOptionMyAdFP']) {
        case "lowest":
            $orderCondition = ' ORDER BY price ASC, ID';
            break;
        case "highest":
...
$latestRecords = $stmt2->fetchAll(PDO::FETCH_ASSOC);
  ...
  $stmt_total = $pdo->prepare("SELECT COUNT(*) AS total_records FROM house WHERE ID IN ($implodedIds)");
    $stmt_total->execute();
...
<li class="nav-item">
  <a class="nav-link text-dark" aria-current="page" href="MyFavouriteCar.php" >Car</a>
</li>
<li class="nav-item">
  <a class="nav-link " href="MyFavouriteProperty.php" style="color:#074eac;">Property</a>
</li>
              ...
              <button
               ...
               <p class="text-dark fw-bold py-2 m-0 text-start">Sort By</p>
                ...
                foreach ($sorts as $sort) {
                    $checked = (!empty($_SESSION['userSortOptionMyAdFP']) && $_SESSION['userSortOptionMyAdFP'] === $sort) ? 'checked' : '';
                    echo "
