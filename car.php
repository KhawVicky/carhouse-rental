...
// 构建查询语句
$stmt = $pdo->prepare('SELECT SQL_CALC_FOUND_ROWS * FROM car WHERE verify = true AND available = true ORDER BY time DESC, ID LIMIT :records_per_page OFFSET :offset');
...
$query = "SELECT SQL_CALC_FOUND_ROWS * FROM car WHERE verify = true AND available = true ";

// Save filter conditions in session
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['search'])) {
    $_SESSION['location'] = !empty($_POST['location']) ? $_POST['location'] : '';
    $_SESSION['carBrand'] = !empty($_POST['carBrand']) ? $_POST['carBrand'] : '';
    $_SESSION['carCategory'] = !empty($_POST['carCategory']) ? $_POST['carCategory'] : '';
    $_SESSION['price'] = !empty($_POST['price']) ? $_POST['price'] : '';
    /*header("Location: http://localhost/CNH-2/car.php?page=1");
exit;*/
}
...
if (!empty($_SESSION['location'])) {
    $query .= " AND location LIKE '%" . $_SESSION['location'] . "%'";
    $locationC=$_SESSION['location'];
}

if (!empty($_SESSION['carBrand'])) {
    $query .= " AND brand LIKE '%" . $_SESSION['carBrand'] . "%'";
}

if (!empty($_SESSION['carCategory'])) {
    $query .= " AND category LIKE '%" . $_SESSION['carCategory'] . "%'";
}

if (!empty($_SESSION['price'])) {
    switch ($_SESSION['price']) {
        case 'RM0-RM100':
            $query .= " AND (price >= 0 AND price <= 100)";
            break;
        case 'RM101-200':
            $query .= " AND (price >= 101 AND price <= 200)";
            break;
        case 'RM201':
            $query .= " AND price >= 201";
            break;
  ...
  <section id="carSection" class="carSection">
      <div class="container py-5">
        <div class="row">
         ...
         <div class="row">
           <div class="col-3">
           <p class="text-dark ">Car For Rent In <?=$locationC?> |<br>Found <b><?=$total_records?></b> Results.</p>  
            <!--Location-->
              ...
              <button
               ...
               <p class="text-dark fw-bold py-2 m-0 text-start">Location</p>
                ...
                foreach ($locations as $location) {
                    $checked = (!empty($_SESSION['location']) && $_SESSION['location'] === $location) ? 'checked' : '';
                    echo "
            ...
            <div class="row bg-light py-3 shadow-lg mb-4 rounded-3">
              ...
              <button
                ...
                aria-label="toggle navigation"
                 >
               <p class="text-dark fw-bold py-2 m-0 text-start">Brand Model</p>
              </button>
              </div>
        ...
        foreach ($brands as $brand) {
            $checkedBrand = (!empty($_SESSION['carBrand']) && $_SESSION['carBrand'] === $brand) ? 'checked' : '';
            echo "
                    <li class='nav-item text-dark'>
                    <label class='nav-link px-3 filterBar' style='cursor:pointer;'>
                    <input type='radio' name='carBrand' style='cursor:pointer;' value='$brand' $checkedBrand> $brand</label>
                    </li>";
                }
        ...
        ?>
            ...
            <div class="row bg-light py-3 shadow-lg mb-4 rounded-3">
              ...
              <button
                ...
                aria-label="toggle navigation"
                 >
               <p class="text-dark fw-bold py-2 m-0 text-start">Category</p>
              </button>
              </div>
              <div class="collapse  " id="navbarNavDropdown3">
                <ul class="nav navbar-nav me-auto px-2">
                <?php
        $carCategoryOptions = ['Small','SUV','Luxury'];

       foreach ($carCategoryOptions as $category) {
          $checkedCate = (!empty($_SESSION['carCategory']) && $_SESSION['carCategory'] === $category) ? 'checked' : '';
          echo "
          <li class='nav-item text-dark'>
          <label class='nav-link px-3 filterBar' style='cursor:pointer;'>
          <input type='radio' name='carCategory' style='cursor:pointer;' value='$category' $checkedCate> $category</label>
          </li>";
       }
    ...
    ?>
        ...
        $priceRanges = [
          'RM0-RM100' => 'RM0-RM100',
          'RM101-200' => 'RM101-200',
          'RM201' => 'RM201 and above'
      ];
      foreach ($priceRanges as $value => $label) {
        $checkedPrice = (!empty($_SESSION['price']) && $_SESSION['price'] === $price) ? 'checked' : '';
