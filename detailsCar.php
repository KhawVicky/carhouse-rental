<?php
    ...
    $loginFirst=false;
      ...
      $email=$latestRecords[0]['email'];  //提取这个Ad的email
      ...
      $stmt = $pdo->prepare('SELECT * FROM bookmark WHERE email = :email AND relatedId	=:relatedId AND category="car"');
      ...
      if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if (isset($_POST['recordID']) && isset($_POST['status'])) {
          ...
          if ($status === 'Available') {
            ...
            $stmt->execute([$id,$emailBookMark,"car",$recordID]);
        } elseif ($status === 'UnAvailable') {
          $stmt = $pdo->prepare('DELETE FROM bookmark WHERE relatedID = ? AND email=? AND category="car"');
                    ...
                    <div class="tab-content" id="myTabContent">
                      ...
                      <div class="tab-pane fade show active" id="tab1" role="tabpanel" aria-labelledby="tab1-tab">
                        <ul class="list-unstyled">
                          ...
                          <li class="border-bottom px-3 py-2 d-flex justify-content-between">
                            <span class="py-2">
                              <strong> <i class="fa-solid fa-font-awesome"></i> Car Brand :</strong>
                            </span>
                            <span class="py-2"><?=$latestRecords[0]['brand']?></span>
                          </li>
                          <li class="border-bottom px-3 py-2 d-flex justify-content-between">
                            <span class="py-2">
                          ...
                          <li class="border-bottom px-3 py-2 d-flex justify-content-between">
                            ...
                            <span class="py-2"><?=$latestRecords[0]['category']?></span>
                          </li>
                          <li class="border-bottom px-3 py-2 d-flex justify-content-between">
                            <span class="py-2">
...
function updateAvailability(recordID, status) {
    ...
    xhr.open("POST", "detailsCar.php"); // Point to the correct PHP file
